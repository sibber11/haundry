...
<div class="grid grid-cols-1 sm:grid-cols-2 sm:bg-gradient-to-l sm:from-white sm:to-transparent sm:via-gray-200">
    <form action="{{route('post_feedback')}}" method="post">
        ...
        <div class="mt-1 flex flex-col gap-2">
            ...
            <input type="text" name="name" id="contact-name" class="p-2 rounded" value="{{old('name')}}">
        </div>
        <div class="mt-1 flex flex-col gap-2">
            <label for="contact-email ">Email: </label>
            <input type="email" name="email" id="contact-email" class="p-2 rounded" value="{{old('email')}}">
        </div>
        <div class="mt-1 flex flex-col gap-2">
            <label for="contact-info ">Your Feedback: </label>
            <textarea name="feedback" cols="30" rows="10" id="contact-info" class="p-2 rounded">
                {{old('feedback')}}
            </textarea>
        </div>
        <div class="mt-2">
